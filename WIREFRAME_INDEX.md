...
### 수량구간 가격 (DEMO — N-150 / TTC 니퍼 150mm / EA 기준)

| 구간 | 수량 범위 | 단가 | 현재 구간 표시 기준 |
|---|---|---|---|
| 구간 1 | 1 ~ 9 EA | ₩12,800 | 수량 1~9 입력 시 강조 |
| 구간 2 | 10 ~ 49 EA | ₩11,500 | 수량 10 이상 입력 시 강조 |
| 구간 3 | 50 EA 이상 | ₩10,200 | 수량 50 이상 입력 시 강조 |

> **규칙**: 수량 10개 이상 화면에서는 현재 적용단가를 **₩11,500**으로 표시한다. 현재 적용단가를 가장 크게 표시하고 다른 구간은 보조 정보로 표현한다.
...
| 적용단가 | ₩12,800 (1~9 구간) | 수량 20은 구간 2이므로 ₩11,500 적용 주의 |

> WF-06에서 적용단가 ₩12,800 → 검토 결과 20개이므로 구간 2 적용 → **₩11,500**이 정합성 기준.
> (WF-05의 수량 5 예시는 구간 1 → ₩12,800 적용 — 별도 수량 예시)

**품목 B: TTC 드라이버 PH#2 (DS-PH2)**
...
**구매자 화면 표시**: 확정 50개 · 즉시출고 30개 · 백오더 20개

> [!IMPORTANT]
> backordered_quantity는 accepted_quantity에 포함된 미출고 수량입니다.
> "30개 수락 + 20개 백오더" 형태로 표시하지 않습니다.
...
| **확정 주문 소계** | **₩520,000** | 백오더 포함 전체 accepted 기준 |
| 즉시출고 예정금액 | ₩174,000 | DS-PH2 30 × ₩5,800 |
| 백오더 금액 | ₩116,000 | DS-PH2 20 × ₩5,800 |
| 부가세 표시 | DEMO (tax_exclusive 예시) | VAT 정책 미확정 (OD-005~008) |

> **VAT 주의**: 와이어프레임의 세액 숫자는 "DEMO 세금표시 예시 (tax_exclusive 예시)"임을 명시. "부가세 10%" 단정 금지.
